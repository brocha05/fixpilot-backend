...
export class AnalyticsService {
  ...
  async getRevenueSummary(
    ...
    const pendingOrders = await this.prisma.repairOrder.findMany({
      where: {
        companyId,
        status: {
          notIn: [RepairStatus.COMPLETED, RepairStatus.DELIVERED, RepairStatus.CANCELLED],
        },
      },
  ...
  async getRepairStats(companyId: string): Promise<RepairStats> {
    ...
    const completed = byStatus[RepairStatus.COMPLETED] + byStatus[RepairStatus.DELIVERED];
  ...
  async getDashboardSummary(companyId: string): Promise<DashboardSummary> {
    ...
    const netProfit = Math.round((revenue.totalRevenue - expenses.totalExpenses) * 100) / 100;
