...
export class RepairEventListeners {
  ...
  async onStatusChanged(event: RepairStatusChangedEvent): Promise<void> {
    ...
    try {
      ...
      const newStatusLabel = REPAIR_STATUS_LABELS_ES[event.newStatus] ?? event.newStatus;
  ...
  async onApprovalRequested(event: RepairApprovalRequestedEvent): Promise<void> {
    if (!event.customerEmail) return;
    ...
    try {
      ...
      this.logger.error('onApprovalRequested handler failed', (err as Error).stack);
    }
  }
