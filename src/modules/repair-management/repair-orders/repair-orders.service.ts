...
export class RepairOrdersService {
  ...
  async create(
    ...
    this.logger.log(`RepairOrder created: ${order.id} for company: ${companyId}`);
  ...
  async findAll(
    ...
    const { page = 1, limit = 20, skip, status, urgencyLevel, customerId, fromDate, toDate } =
      filters;
  ...
  async findById(id: string, companyId: string): Promise<RepairOrderResponseDto> {
    const order = await this.prisma.repairOrder.findFirst({
      where: { id, companyId },
      include: DETAIL_INCLUDE,
    });
    if (!order) throw new NotFoundException('Orden de reparación no encontrada');
  ...
  async update(
    ...
    if (order.status === RepairStatus.DELIVERED || order.status === RepairStatus.CANCELLED) {
  ...
  async changeStatus(
    ...
    if (dto.status === RepairStatus.WAITING_APPROVAL) {
      ...
      event.costEstimate = updated.costEstimate ? Number(updated.costEstimate) : null;
  ...
  async addComment(
    ...
    companyId: string,
    userId: string,
  ): Promise<{ id: string; message: string; internal: boolean; createdAt: Date }> {
  ...
  async removeImage(id: string, imageId: string, companyId: string): Promise<void> {
    await this.findOrderOrThrow(id, companyId);
  ...
  async findByToken(token: string) {
    ...
    if (!order) throw new NotFoundException('Token de seguimiento inválido o no encontrado');
