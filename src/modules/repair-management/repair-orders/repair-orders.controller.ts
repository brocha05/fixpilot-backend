...
export class RepairOrdersController {
  ...
  findAll(@CurrentUser() user: JwtPayload, @Query() filters: ListRepairOrdersDto) {
    return this.repairOrdersService.findAll(user.companyId, filters);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Obtener detalle completo de una orden (con historial, comentarios e imágenes)' })
  findOne(
  ...
  @Patch(':id/status')
  @ApiOperation({ summary: 'Cambiar estado de la orden (aplica reglas de transición estrictas)' })
  ...
  changeStatus(
    ...
    return this.repairOrdersService.changeStatus(id, dto, user.companyId, user.sub);
  }
  ...
  @Post(':id/comments')
  @HttpCode(HttpStatus.CREATED)
  @ApiOperation({ summary: 'Agregar un comentario a la orden (puede ser interno o público)' })
  addComment(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: AddCommentDto,
    @CurrentUser() user: JwtPayload,
  ) {
    return this.repairOrdersService.addComment(id, dto, user.companyId, user.sub);
  }
  ...
  @ApiBody({ schema: { properties: { fileKey: { type: 'string' } }, required: ['fileKey'] } })
  addImage(
    @Param('id', ParseUUIDPipe) id: string,
