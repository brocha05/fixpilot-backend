...
function makeOrder() {
  return {
    ...
    statusHistory: [
      ...
      { newStatus: RepairStatus.IN_PROGRESS, timestamp: new Date('2024-01-02') },
    ],
    comments: [
      // Only internal: false comments come from the DB query in findByToken
      { message: 'Revisión inicial completada', createdAt: new Date('2024-01-01') },
    ],
  };
}
...
const mockPrisma = {
  ...
  repairOrderImage: { findFirst: jest.fn(), create: jest.fn(), delete: jest.fn() },
  $transaction: jest.fn(),
};
...
describe('RepairOrdersService — Public Tracking', () => {
  ...
  describe('findByToken', () => {
    ...
    it('throws NotFoundException for an unknown token', async () => {
      ...
      await expect(service.findByToken(VALID_TOKEN)).rejects.toBeInstanceOf(NotFoundException);
    });
